...
let bodyEl = document.body,
	...
	timerInterval,
	correctAnswersArray = [
		2, 0, 4, 4, 1, 0, 1, 1, 1, 5, 3, 0, 3, 6, 1, 2, 0, 5, 4, 7, 3, 3, 6, 5,
		3, 6, 6, 2, 1, 7,
	];
...
function questionBox() {
	if (currentQuestion <= 30) {
		...
		const numberOfQuestions = currentQuestion < 13 ? 7 : 9;
		// answers function
...
function IQStatus() {
	if (correct >= 25) {
		...
		checkLanguage(
			"Your IQ is the lowest level (Cognitively impaired)",
			"ضریب هوشی شما پاییت ترین حالت ممکن است"
		);
	}
...
function correctAnswers() {
	...
	wholeQuestionBox[0].children[1].children[
		correctAnswersArray[currentQuestion - 1]
